...
Выполнив данную работу, студент сможет подготовить образ контейнера для запуска веб-сайта на базе *Apache HTTP Server + PHP + MariaDB*.

## Задание

Создать Dockerfile для сборки образа контейнера, который будет содержать веб-сайт на базе *Apache HTTP Server + PHP + MariaDB*. База данных *MariaDB* должна храниться в монтируемом томе. Сервер должен быть доступен по порту 8000.
...
В ходе работы был запущен сайт на **WordPress**, проведена настройка всех необходимых компонентов, включая установку модулей, экспорт/импорт конфигурационных файлов и настройку базы данных. В результате сайт успешно работает, что подтверждено на скриншотах. Были приобретены навыки работы с **WordPress**, конфигурацией БД и серверных файлов.
...
Для более тщательного изучения работы томов в **Docker**, мною был создал еще один контейнер, который должен был подключиться к базе данных, уже сконфигурированной первым контейнером, для хранения данных wordpress. Таким образом, сайт можно будет запустить с любого из них, и все изменения будут также сохранены и доступны при запуске с любого из них.
![Volumes](images/Volumes.png)

Однако так как при создании первого контейнера тома создавались автоматически, с произвольными названиями, то взаимодействие с ними происходит не самым удобным образом, а именно приходится самому извлекать их названия, при создании нового контейнера. Также названия не соответствуют стандарту, а именно *Self-documenting*.
